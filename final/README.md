...
| `final_finch/`       | A Python directory that contains code for your Dagster repository                 |
| `final_finch_tests/` | A Python directory that contains tests for `final_finch`                      |
...
pyenv virtualenv $PYTHON_VERSION final_finch
pyenv activate final_finch
```

**Conda**
```bash
export PYTHON_VERSION=X.Y.Z
conda create --name final_finch python=PYTHON_VERSION
conda activate final_finch
```
...
Tests can be found in `final_finch_tests` and are run with the following command:

```bash
pytest final_finch_tests
```

As you create Dagster ops and graphs, add tests in `final_finch_tests/` to check that your
